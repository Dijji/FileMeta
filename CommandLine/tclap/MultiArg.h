...
bool MultiArg<T>::processArg(int *i, std::vector<std::wstring>& args) 
{
	...
	std::wstring value = __T("");

   	trimFlag( flag, value );

   	if ( argMatches( flag ) )
   	{
   		if ( Arg::delimiter() != __T(' ') && value == __T("") )
			throw( ArgParseException( 
			           __T("Couldn't find delimiter for this argument!"),
					   toString() ) );
		...
		if ( value == __T("") )
		{
			...
			if ( static_cast<unsigned int>(*i) < args.size() )
				...
				throw( ArgParseException(__T("Missing a value for this argument!"),
                                         toString() ) );
		} 
		else
			_extractValue( value );
...
std::wstring MultiArg<T>::shortID(const std::wstring& val) const
{
	...
	return Arg::shortID(_typeDesc) + __T(" ... ");
}
...
{
	static_cast<void>(val); // Ignore input, don't warn
	return Arg::longID(_typeDesc) + __T("  (accepted multiple times)");
}
